...
#define CONTROLS_GAP_Y                      (15)       // gap by X coordinate
//--- for buttons
...
bool CreateLabel_HTF_Alert(void) {
  ...
  if(!m_label1.Create(0,"Label_HTF_Alert",0,x1,y1,x2,y2))
    return(false);
  if(!m_label1.Text("HTF WatchList"))
    return(false);
...
bool CreateLabel_H1_Alert(void) {
  ...
  int y2=y1+LABEL_HEIGHT;   // y2 = 11 + 20  = 32  pixels
  //--- create
  if(!m_label2.Create(0,"Label_H1_Watchlist",0,x1,y1,x2,y2))
    return(false);
  if(!m_label2.Text("H1 WatchList"))
    return(false);
...
bool CreateLabel_LTF_Alert(void) {
  ...
  if(!m_label3.Create(0,"Label_LTF_Alert",0,x1,y1,x2,y2))
    return(false);
  if(!m_label3.Text("LTF WatchList"))
    return(false);
...
bool IsNewBar3(const string symbol, const ENUM_TIMEFRAMES period)
{
  ...
  {
    // Don't want new bar just because EA started, we want a new bar instead
