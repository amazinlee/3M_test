                ...
                const int &spread[] ) 
{
  ...
  for(int i = start; i < iEnd_Of_Arr && !IsStopped(); i++) {  //don't include the current bar rates_total-1 is the current bar.
    ...
    if ((MathAbs(rates[i].high-rates[i].low))<0.8*ATR[i]) {continue;} //bar must be of min height in relative to ATR
    ...
    if (sSignal=="Up1") {
      UpArrow[iArrowPos] = rates[i].low;
      //UpX[iArrowPos] = rates[i].low + arrowbuffer;
    }
    if (sSignal=="Down1") {
      DownArrow[iArrowPos] = rates[i].high; 
      //DownX[iArrowPos] = rates[i].high-arrowbuffer;
    }
    //UpX[iArrowPos] = EMA5[i] + arrowbuffer;
    //DownX[iArrowPos] = EMA5[i]-arrowbuffer;
  } //for loop
