...
//input double GradientFilter=0.2;
double GradientFilter=0.7;
input double Counter_8_21_42_Filter=0.35;

//ATF factor set to 3 to catch the recent ATR, so more buffer for volatility
                ...
                const int &spread[] ) 
{
  ...
  for(int i = start; i < rates_total-1 && !IsStopped(); i++) {  //don't include the current bar rates_total-1 is the current bar.
    datetime StartDt=D'2021.02.05 13:00', EndDt=D'2021.02.05 15:00';
    ...
    if (debug) {
      ...
      double score=MA_Score(i, EMA5[i]);
      if (score>=MinUpDown) {
        Up1[i] = EMA13[i] - arrowbuffer;
      }
      else if (score<=-MinUpDown) {
        Down1[i] =  EMA13[i] + arrowbuffer;
      }
    }//else
  } //for loop
...
//if MA is near ema5 it will get a higher weightage
  ...
  MA_Tan3[5]= (SMA42[pos] - SMA42[pos-1]) / ATR[pos];
  ...
  MA_Tan2[5]= (SMA42[pos-1] - SMA42[pos-2]) / ATR[pos];
  ...
  Print("Tan ",MA_Tan[2]/LookBack, " ", MA_Tan3[2]);

  float score=0, total_score=0;
  //for scoring of MA slopes up, start from sma 31, skip ema 5 and sma 8
  for (int i=2; i<=2; i++) { 
    //there must be an drop in ROC
    if ((MA_Tan1[i] > Gradient1) && (MA_Tan3[i] < MA_Tan2[i]) && (MA_Tan2[i] < MA_Tan1[i]) && (MA_Tan[i]<GradientFilter)) {
    //if ((MA_Tan1[i] > Gradient1) && (MA_Tan[i]<GradientFilter)) { //for 3 bars not steep enough
    //if ((MA_Tan1[i] > Gradient1)) { //for 3 bars not steep enough
      score=1;
    } 
    else {
      score=0;
    ...
    if (MA_Tan1[i]<0 && MA_Tan2[i]<0) { 
      double MAdown1= MathAbs(MA_Tan1[i]);
      double MAdown2= MathAbs(MA_Tan2[i]);
      double MAdown3= MathAbs(MA_Tan3[i]);
      if (MAdown1 > Gradient1 && (MAdown3 < MAdown2) && ( MAdown2 < MAdown1) && (MA_Tan[i]>-GradientFilter)) {
//      if (MAdown1 > Gradient1 && ( MAdown2 < MAdown1)) {
        score=-1;
      }
      else {
        score=0;
    ...
    total_score= total_score+score; //total up the scores of all MA, with gradient2 and 3 get additional score
...
//  if (ScoreUp>=MinUpDown && CounterUp<=AllowCounterMA && total_score>MinScore && R1_Slope && AllowUp) {
  if (ScoreUp>=MinUpDown) {
    iResults=ScoreUp;
  }
//  else if (ScoreDown<=-MinUpDown && CounterDown<=AllowCounterMA && total_score<-MinScore && R1_Slope && AllowDown) {
