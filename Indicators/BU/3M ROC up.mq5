                ...
                const int &spread[] ) 
{
  ...
  for(int i = start; i < rates_total-1 && !IsStopped(); i++) {  //don't include the current bar rates_total-1 is the current bar.
    datetime StartDt=D'2020.10.29 18:00', EndDt=D'2020.10.29 23:00';
...
//if MA is near ema5 it will get a higher weightage
  ...
  for (int i=1; i<=2; i++) {
    ...
    if ((MA_Tan3[3] > Gradient21) && (MA_Tan3[4] > -0.04) && (MA_Tan3[i] > Gradient1) && (MA_Tan3[i] > MA_Tan2[i]) && (MA_Tan2[i] > MA_Tan1[i])) {
      score=1;
    } 
    else {
      score=0;
