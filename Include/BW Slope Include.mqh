...
//for Expert Advisor
string Slope_EA(string sSymbol, ENUM_TIMEFRAMES TF, int LookBack, float &atr) {
  ...
  for (int i=1; i<=LookBack; i++) {
    sResSub=fnBW_Slope(pos-i, sSymbol);
  ...
  if (iPastSigAll==LookBack || iPastSigNone==LookBack)
    sRes=fnBW_Slope(pos, sSymbol); //check current bar for signal
  else
...
string fnBW_Slope(int pos, string sSymbol) {
  string sResults; 
  double atr=ATR[pos];
  int iPreventSig=0;
  if (atr==0) return 0;
