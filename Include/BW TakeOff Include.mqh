...
//for Expert Advisor
string TakeOff(string sSymbol, ENUM_TIMEFRAMES TF, int LookBack, float &atr) {
  ...
  string sSignal="", sRes, sResSub;
  int iPastSigAll=0, iPastSigNone=0;
  //pass by ref
  atr=ATR[pos];

  //check lookback for past few bars if they have signals, if there are signals we want to skip
  for (int i=1; i<=LookBack; i++) {
    sResSub=fnTakeOff(pos-i, sSymbol);
    if (sResSub!=NULL) iPastSigAll=iPastSigAll+1;  //if all bars have signals, we allow current signal to show
    if (sResSub==NULL) iPastSigNone=iPastSigNone+1;  //if none of past have signals, we allow current signal to show
  }

  if (iPastSigAll==LookBack || iPastSigNone==LookBack)
    sRes=fnTakeOff(pos, sSymbol); //check current bar for signal
  else
    sRes=NULL; //do not show signal
...
string fnTakeOff(int pos, string sSymbol) {
  ...
  if ( BWup(pos) && ScoreMAUp(pos,atr) && b13_higher_R1 && b21_higher_R1 && bDist8_21 && bDist21_42 && bDist21_63 && bDist84_126 && bDist42_63 
  && CheckNearUp(pos,atr) && CheckBackUp(pos,atr) && (bPriceCtrl) ) {
    sResults="Up";
  }
  if ( BWdown(pos) && ScoreMADown(pos,atr) && b13_lower_R1 && b21_lower_R1 && bDist8_21 && bDist21_42 && bDist21_63 && bDist84_126 && bDist42_63 
...
bool ScoreMAUp(int pos, double atr) {
  ...
  for (int i=3; i<=6; i++) { //21 to R1, 3 of the 4 MAs should be steep enough to get score of 6
    ...
    if ( (i==3) && MA_Tan[i]<-slope21 ) { iScore=iScore-7; }  //21 can't slope too much against
...
bool ScoreMADown(int pos, double atr) {
  ...
  for (int i=3; i<=6; i++) { //21 to R1, 3 of the 4 MAs should be steep enough to get score of 6
    ...
    if ((i==3) && MA_Tan[i]>slope21) { iScore=iScore-7; }
...
bool CheckBackUp(int pos, double atr) {
  ...
  for (int i=0; i<=12; i++) {
    ...
    //if (debug && debugUp) Print ("Checkback: ",rates[pos-i].time," ", RoundN(dist), " ", RoundN(dist42));
...
bool CheckBackDown(int pos, double atr) {
  ...
  for (int i=0; i<=12; i++) {
    double dist=(rates[pos].close-rates[pos-i].close)/atr;  //for up trades the price can't come down from too high
    double dist42=(SMA42[pos-i]-SMA8[pos-i])/atr;  //for up trades the price can't come down from too high    
    if (debug && debugDn) Print ("CheckbackDown: ",rates[pos-i].time," ", RoundN(dist), " ", RoundN(dist42));
...
bool BWup(int pos) {
  bool bRes=false;
  double barHgt1, barHgt2, BarHgt=0.04;
  barHgt1=MathAbs(rates[pos-1].close - rates[pos-1].open);
  barHgt2=MathAbs(rates[pos].close - rates[pos].open);
  
  if ( barHgt1>BarHgt && barHgt2>BarHgt && (rates[pos-1].close < rates[pos-1].open) && (rates[pos].open < rates[pos].close) ) {  //black then white bar
    bRes=true;
  }
  if (debug && debugUp) Print ("BWup: ", bRes, " ", barHgt1 );
  return bRes;
}
...
bool BWdown(int pos) {
  ...
  if ( barHgt1>BarHgt && barHgt2>BarHgt && rates[pos-1].close > rates[pos-1].open && rates[pos].open > rates[pos].close ) {  //white then black bar
    bRes=true;
  }
  if (debug && debugDn) Print ("BWdown: ", bRes);
  return bRes;
}